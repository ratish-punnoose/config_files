...
2. Set the load path
   #+begin_src emacs-lisp
     (defun rjp/add-to-loadpath (dirname)
       "Add given directory and subdirectories to load path."
       (interactive)
       ;; Use the function that works with the default directory
       (when (file-accessible-directory-p dirname)
         (setq tmp default-directory)
         (setq default-directory dirname)
         ;;(setq load-path (append dirname load-path))
         (normal-top-level-add-subdirs-to-load-path)
         (setq default-directory tmp)))
     (rjp/add-to-loadpath (expand-file-name "elpa/" user-emacs-directory))
     (rjp/add-to-loadpath (expand-file-name "lisp/" user-emacs-directory))
   #+end_src
3. Setup up intitial packages
...
4. Prevent packages from being loaded twice
   #+begin_src emacs-lisp :tangle yes
...
* Package Management
1. Enable use-package-always-ensure if you wish this behavior to be global for all packages.
   #+begin_src emacs-lisp :tangle no
   (require 'use-package-ensure)
   (setq use-package-always-ensure t)
   #+end_src
2. use-package is needed to load packages but not needed at run time.
   #+begin_src emacs-lisp
