...
** Calc
   - Enter with base:  base#number
   - twos complement:  base##number. eg. 2##1101
   - Change word size (default 32):  b w.  This can be negative.
   - Change radix for display: d r n.  Shortcuts for common
     - d 0 : decimal
     - d 2 : binary
     - O d 2 : binary twos complement
     - d 6 : hex
   - For example: To convert a twos-complement hex value to decimal
     - O d 6: Hex twos complement.
     - 16##C1F8F181 : Enter this number in twos complement. Note two #s
     - d 0 : Show decimal value.

   - Show leading zeros: d z
   - Changing from signed to unsigned.  Use binary clip.
     1. Set word size: e.g. 8/-8
     2. Clip/reduce: b c
...
** Packages
   ...
   cygcheck-dep -r
   #+end_src
** Speedup (especially over VPN)
   - Cygwin can slow down due to Active Directory.
     - See [[https://stackoverflow.com/questions/28507014/cygwin-commands-from-cmd-are-slow-from-bash-are-fast]]
     - See [[https://cygwin.com/faq/faq.html#faq.using.startup-slow]]
     - Fix
       - Create passwd and group files using mkpasswd -l and mkgroup -l
       - Change nsswitch.conf entries for passwd, group to: files instead of
         "files db".
** X11
*** Running
...
* To wipe free space in windows
  ...
  cipher /w:C
  #+END_EXAMPLE
* Windows Intel Graphics resolution
  For HD 2000, using HDMI, the max resolution is typically 1920x1080.
  With the [[https://community.intel.com/t5/Graphics/External-screen-max-resolution/m-p/627799#:~:text=The%20Maximum%20Display%20Resolution%20for,1920x1080%20via%20HDMI%2FVGA%20only.][following]] [[https://www.notebookcheck.net/2560x1440-or-2560x1600-via-HDMI.92840.0.html][trick]], the resolution can be pushed a bit higher:
  - Advanced Parameters: 2560x1440, 55Hz refresh, 32 bit depth, CVT-RB
    timing.
...
** firewall with firewalld
   ...
   - The files are stored in /etc/firewalld/zones/public.xml
   - To add ports:
     - firewall-cmd --permanent --add-port=portnum/tcp
     - firewall-cmd --reload
** Adding virtual interface
*** Creating interface
    Create file such as /etc/modprobe.d/dummy.conf, with the following text content
    #+BEGIN_EXAMPLE
...
*** Fixing problems
    - systemctl status unitname to see what failed.
      - status=203/EXEC could be due to bad executable.
      - One reason is due to SeLinux settings.
        - Check using ls -lZ filename.
        - Fix as follows for all files under a directory
          - For the whole upper directory
            #+begin_example
              semanage fcontext -a -t bin_t dirname
              restorecon -R -v dirname
            #+end_example
        - If semanage is not installed, look for package: policycoreutils-python-utils
    - lsb
      - On newer redhat (and maybe ubuntu), lsb-core may be missing.
        - Create a symbolic link to ld-lsb-x86-64.so.3 from ld-linux-x86-64.so.2
...
** sssd
   - adding users:  add to sssd.conf. to metagroup specified in ldap_access_filter
...
** Library no such file or directory
   - ldd daemon may give /lib64/ld-lsb-x86-64.so which doesn't seem to be
     present on Redhat 9.
   - sym linking /lib64/ld-linux-x86-64.so.2 to this missing one seems to fix.
* Markup
