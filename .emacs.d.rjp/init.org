...
* Pre-setup before loading packages
  ...
  3. Utility functions used within this file
     1. Get list of directories and subdirectories
        #+begin_src emacs-lisp
          (require 'seq)
          ...
          (defun rjp/get-info-subdirs (dirname)
            ...
            (if (file-accessible-directory-p dirname)
              ...
              nil))
        #+end_src
     2. We also add a utility function to print out time taken to initialize
        #+begin_src emacs-lisp
          (defun rjp/init-time-so-far (msg_to_print)
...
* Package Management
  ...
  See directions and usage of diminish,bind-key at : [[https://github.com/jwiegley/use-package#use-packageel-is-no-longer-needed-at-runtime][use-package site]]
  #+begin_src emacs-lisp
    (eval-when-compile
      (require 'use-package)
      (require 'use-package-hydra)
      )
    ;; As per use-package readme.md , section use-package.el is no longer needed at runtime
  #+end_src
** Other basic packages
   #+begin_src emacs-lisp :tangle yes
     ...
     (require 'hydra)
   #+end_src
   The following can cut the load time by about 0.2 seconds. But not tested
   thoroughly to see if there is any loss of function.
   #+begin_src emacs-lisp :tangle no
     (use-package diminish  :defer 2 )
     (use-package bind-key  :defer 2   )
...
** Default to non tabs mode
   ...
   #+end_src
** Increase column width before auto-wrapper
   #+begin_src emacs-lisp
   ...
   #+begin_src emacs-lisp
     ...
     (cond ((or (eq system-type 'windows-nt)
            ...
            (custom-set-faces
             ...
             '(fixed-pitch ((t (:family "Consolas" :foundry "outline" :slant normal
                                        :weight normal :height 113 :width normal))))
             '(fixed-pitch-serif ((t (:family "Consolas" :foundry "outline" :slant normal
                                              :weight normal :height 113 :width normal))))
             ))
...
** Line number display
   #+begin_src emacs-lisp
     (custom-set-faces '(line-number ((t (:foreground "DimGray"))))
                       '(line-number-current-line ((t (:foreground "LimeGreen")))))
   #+end_src
* Navigation
  :PROPERTIES:
  :header-args: :tangle yes
...
* Use string syntax for re-builder
  #+begin_src emacs-lisp
    (custom-set-variables '(reb-re-syntax 'string))
  #+end_src
...
** Tab in code block
   ...
   #+end_src
** Org indentation
   - org-indent shows indentation virtually but it is not reflected in
     the file.
   - To reflect in the file:
     - turn org-indent off.
     - set org-adapt-indentation to t:
     - set electric-indent-mode to t.
   #+begin_src emacs-lisp
     (custom-set-variables  '(org-adapt-indentation t))
   #+end_src
...
** Unprettify symbol at point
   ...
   #+END_SRC
** Poporg mode to pop out comments into org buffer
   :PROPERTIES:
   :header-args: :tangle yes
   :END:
   #+begin_src emacs-lisp
...
* Magit
  ...
  #+begin_src emacs-lisp
    (use-package magit
      ...
      :config
      (magit-add-section-hook 'magit-status-sections-hook 'magit-insert-modules-overview 'magit-insert-modules-unpulled-from-upstream)
      ;;:config
      ;;(require 'magit-blame)
      ;;(setq magit-refresh-status-buffer nil)
    ...
    #+begin_src emacs-lisp
      (use-package eshell
        ...
        (defun eshell/zgrep (&rest args)
          "Use Emacs grep facility instead of calling external grep."
          (eshell-grep "zgrep --color -H" args))
  ...
  - Remove up/down arrow from eshell-mode-map
    ...
    #+begin_src emacs-lisp
      (use-package em-hist
        ...
        (define-key eshell-hist-mode-map [C-down] nil)
        (define-key eshell-hist-mode-map [C-up] nil)
        (define-key eshell-hist-mode-map [up] nil)
        (define-key eshell-hist-mode-map [down] nil)
        (define-key eshell-hist-mode-map [tab]
                    'rjp/eshell-outline-toggle)
        (define-key eshell-hist-mode-map [S-tab] 'rjp/eshell-outline-global-toggle)
     ...
     #+begin_src emacs-lisp
       (defun rjp/bookmark-rev-search (filename)
         ...
         (when (boundp 'bookmark-alist )
           (cl-dolist (entry bookmark-alist)
             (when
                 (string=
    ...
    #+begin_src emacs-lisp
      (use-package artist
        ...
        (ryo-modal-major-mode-keys
         ...
         ("K" artist-select-op-cut-rectangle)
         ("k" artist-select-op-copy-rectangle)
...
* Workspaces
  ...
  :END:
  #+begin_src emacs-lisp
    (defun rjp/project-override (dir)
      (let ((override (locate-dominating-file dir ".project.el")))
        (if override
            (cons 'vc override)
          nil)))
    (use-package tabspaces
      :init (use-package project
              :config
              (add-hook 'project-find-functions #'rjp/project-override))
...
** Key selection examples
   - [[https://gist.github.com/LLazarek/d9c226763860c3f53a6d28535e9efb64][nav-mode]] similar to vim's normal mode
   - [[https://www.spacemacs.org/doc/DOCUMENTATION.html#commands][Spacemacs key bindings]]
   - [[https://devhints.io/spacemacs][Spacemacs cheat sheet]]
   - [[https://github.com/jmorag/kakoune.el/blob/master/kakoune.el#L51][Kakoune]]
   - [[https://github.com/mawww/kakoune/blob/master/doc/pages/keys.asciidoc#goto-commands][Kakoune Keys]]
   -
